...
import { LogOut, Globe, User, ChevronRight } from "lucide-react";
    ...
    <aside className="hidden lg:flex w-72 bg-white border-r border-zinc-100 flex-col h-screen sticky top-0">
      <div className="p-8">
        <Link href="/admin" className="flex items-center gap-3 group">
          <div className="bg-zinc-950 text-white w-10 h-10 flex items-center justify-center rounded-xl text-xs font-black shadow-xl shadow-zinc-500/20 group-hover:scale-110 transition-transform">
            B2B
          </div>
          <div>
            <h2 className="text-sm font-black text-zinc-900 uppercase tracking-widest leading-none">Console</h2>
            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-1">Admin Access</p>
          </div>
        </Link>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="mx-2 p-3 bg-zinc-50 rounded-[1.5rem] border border-zinc-100 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-zinc-950 text-white flex items-center justify-center text-xs font-black shadow-lg">
            {user?.username?.[0]?.toUpperCase() || "A"}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-black text-zinc-900 truncate uppercase tracking-widest">{user?.username || "Admin"}</p>
            <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{user?.role || "System"}</p>
          </div>
        </div>
